...
function createViewTask(view: View) {
    ...
    viewTask.setAction(async (args, hre) => {
        ...
        const token = await hre.ethers.getContractAt("ERC20", args.contract);
...
task("mint", "Transfer token to address")
    ...
    .setAction(async (args, hre) => {
        ...
        const tx = ((await token.connect(sender)) as any).mint(
            args.recepient,
            args.amount
        );
...
task("burn", "Transfer token to address")
    ...
    .setAction(async (args, hre) => {
        ...
        const tx = await (token.connect(sender) as any).burn(
            args.target,
            args.amount
        );
